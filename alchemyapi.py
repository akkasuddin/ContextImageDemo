...
class AlchemyAPI:
    ...
    ENDPOINTS['facedetection']['url'] = '/url/URLGetRankedImageFaceTags'
    ENDPOINTS['facedetection']['image'] = '/image/ImageGetRankedImageFaceTags'
...
#Zeeshan :Facetagging
    def faceDetection(self, flavor, data, options={}):
        """
        ...
        options -> various parameters that can be used to adjust how the API works, see below for more info on the available options.
        """
        if flavor not in AlchemyAPI.ENDPOINTS['facedetection']:
            return {'status': 'ERROR', 'statusInfo': 'facedetection for ' + flavor + ' not available'}
        elif 'image' == flavor:
            image = open(data, 'rb').read()
            options['imagePostMode'] = 'raw'
            options['knowledgeGraph'] = 1
            return self.__analyze(AlchemyAPI.ENDPOINTS['facedetection'][flavor], options, image)

        options['knowledgeGraph'] = 1
        options[flavor] = data
        return self.__analyze(AlchemyAPI.ENDPOINTS['facedetection'][flavor], {}, options)
#Zeeshan :Facetagging
